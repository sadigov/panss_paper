...
for (i in 1:n){
    ...
    theta[i, 1:T] <- (theta.raw[i, 1:T] - shift) / scale
}
...
filename <- file.path(getwd(), "lgrmar2.txt")
## write model file:
write.model(lgrmar2, filename)
file.show(filename)
...
data <- list("Y", "trt", "n", "p", "T", "K", "m.kappa", "s.kappa", "m.alpha", "s.alpha", "m.mu.theta", "s.mu.theta", "n.trt")
...
lgrmar2.out <- R2OpenBUGS::bugs(data = data, inits = NULL,
