...
data {
  ...
  int K[J, T];                   // max level for each item
...
parameters {
    ...
    real<lower=-1.0, upper=1.0> rho;         // AR(1) parameter
  ...
  for (t in 1:T){
    for (i in 1:N) {
      for (j in 1:J) {
        ...
        p[i, j, t, K[j, t]] <- 1 - Q[i, j, t, K[j, t] - 1];
...
#                 Max level same  at each time point
##########################################################################################
bito_stan_long2 <- '
data {
  real m_mu_theta;               // prior for mean of theta
...
parameters {
    ordered[7-1] kappa[J];                  // intercept : array of ordered vectors
    vector[T] theta[N];                     // ability : theta[N,T] - array of vectors
    real<lower=0.0> alpha[J];               // discrimination
...
model{
   ...
   for (j in 1:J) {
         kappa[j] ~ normal(0.0, 2.5);
      }

// prior for mu_theta
   mu_theta[1] <- 0.0;  // fix to 0 for identifiability
   for (t in 1:(T-1))
      mu_theta[t+1] <- mu_theta_raw[t];

   mu_theta_raw ~ normal(m_mu_theta, var_mu_theta);
...
#
#                  Compound symmetry, population-averaged model
#                 Max level different at each time point
##########################################################################################
...
data {
  ...
  int<lower=0> N;                // number of subjects
  int<lower=0> J;                // number of items
  int<lower=1, upper=7> K[J, T]; // max level for each item at each time point
  int<lower=1, upper=7> maxK[J]; // max level for each item
  int Y[N, J, T];                // data, 3D array of integers
}
...
maxK <- apply(apply(Y, c(2,3), max), 1, max)
K <- apply(Y, c(2,3), max)
# number of arms
...
attr(Y, "dimnames")[3][[1]] <- c("T1","T2","T3","T4","T5","T6","T7" )
...
initf3b <- function() {
    list(theta = array(rnorm(N*T), dim=c(N,T)), alpha = runif(J), kappa = t(apply(array(rnorm(J*6), dim=c(J,6)), 1, sort)), rho= runif(1), mu_theta_raw = rnorm(T-1))
}
fitl3b <- stan(model_code = bito_stan_long2, data = bito_dat, 
               iter = 1000, chains = 3, verbose = FALSE, init = initf3b)
# get the initial values used:
# fitl3b_inits <- get_inits(fitl3b) 
...
print(fitl3b, pars=c("alpha", "rho"), probs=c(.1,.5,.9))
print(fitl3b, pars=c("kappa"), probs=c(.1,.5,.9))
#trace plots
rstan::traceplot(fitl3b, c("alpha"), ncol=2, inc_warmup=F)
rstan::traceplot(fitl3b, c("kappa"), ncol=7, nrow=7, inc_warmup=F)
rstan::traceplot(fitl3b, c("rho"), ncol=7, nrow=7, inc_warmup=F)
