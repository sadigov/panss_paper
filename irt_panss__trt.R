...
for (t in 1:T){
    for (i in 1:n){
        for (j in 1:p){
            ...
            for (k in 1:(K[j] - 1)) {
            logit(P[i, j, t, k]) <- kappa[j, k] - alpha[j]*(theta[i, t]*trt[i])
...
filename <- file.path(getwd(), "lgrmartrt1.txt")
## write model file:
write.model(lgrmartrt1, filename)
file.show(filename)
...
data <- list("Y", "trt", "n", "p", "T", "K", "m.kappa", "s.kappa", "m.alpha", "s.alpha", "m.mu.theta", "s.mu.theta")

monitor <- c("alpha", "kappa", "theta", "rho", "trt")
n.burn <- 4000
n.thin <- 10
n.sim <- 500 * n.thin + n.burn

lgrmartrt1.out <- R2OpenBUGS::bugs(data = data, inits = NULL,
                                 parameters.to.save = monitor,
                                 model.file = file.path(getwd(), "lgrmartrt1.txt"),
                                 n.iter = n.sim, n.thin = n.thin, n.burnin = n.burn,
                                 OpenBUGS.pgm = obd,  working.directory = getwd(),
                                 debug = TRUE,
                                 codaPkg = FALSE)

lgrmartrt1.out <- R2WinBUGS::bugs(data = data, inits = NULL,
                                 parameters.to.save = monitor,
                                 model.file = file.path(getwd(), "lgrmartrt1.txt"),
                                 n.iter = n.sim, n.thin = n.thin, n.burnin = n.burn,
...
lgrmartrt1.out2 <- R2WinBUGS::bugs(data = data, inits = NULL,
                                 parameters.to.save = monitor,
                                 model.file = file.path(getwd(), "lgrmartrt1.txt"),
                                 n.iter = n.sim, n.thin = n.thin, n.burnin = n.burn,
                                 bugs.directory = wbd,  working.directory = getwd(),
