...
for (i in 1:n){
    theta[i, 1:T] ~ dmnorm(mu.theta[trt[i], ], Pr.theta[ , ])
}

## Prior for mu.theta
mu.theta[1 , 1] <- 0.0
mu.theta[2 , 1]  ~ dnorm(m.mu.theta[2], pr.mu.theta)
mu.theta[3 , 1]  ~ dnorm(m.mu.theta[3], pr.mu.theta)

for (t in 2:T){
    for (l in 1:n.trt){
    mu.theta[l, t] ~ dnorm(m.mu.theta[l], pr.mu.theta)

}
}

for (l in 1:n.trt){
    m.mu.theta[l] ~ dnorm(0, 0.0001)
}
...
## #priors on trt effect
## for (l in 1:n.trt){
##     b.trt[l] ~ dnorm(0, 0.0001)
## }
...
filename <- file.path(getwd(), "lgrmar2c.txt")
## write model file:
write.model(lgrmar2c, filename)
file.show(filename)

##########################################################################################
#   Introducing treatment effect (delta)
##########################################################################################
lgrmar2d <- function(){
# Graded response longitudinal model
# Unconstrained - discrimination parameters alpha freely estimated
...
## parameter for treatment effect at each time point, diff in change from baseline at each time period
for (t in 2:T){
    for (l in 1:(n.trt - 1)){
        delta[l, t] <- (mu.theta[l+1, t] - mu.theta[1, t]) - (mu.theta[l+1, 1] - mu.theta[1, 1]) 
    }
}
...
## some temporary filename:
filename <- file.path(getwd(), "lgrmar2d.txt")
## write model file:
write.model(lgrmar2d, filename)
file.show(filename)
...
data <- list("Y", "n", "p", "T", "K", "m.kappa", "s.kappa", "m.alpha", "s.alpha", "trt", "s.mu.theta", "n.trt")
...
lgrmar2c.out <- R2OpenBUGS::bugs(data = data, inits = NULL,
                                 parameters.to.save = monitor,
                                 model.file = file.path(getwd(), "lgrmar2c.txt"),
                                 n.iter = n.sim, n.thin = n.thin, n.burnin = n.burn,
                                 OpenBUGS.pgm = obd,  working.directory = getwd(),
                                 debug = TRUE,
                                 codaPkg = FALSE)

lgrmar2c.out <- R2WinBUGS::bugs(data = data, inits = NULL,
                                 parameters.to.save = monitor,
                                 model.file = file.path(getwd(), "lgrmar2c.txt"),
                                 n.iter = n.sim, n.thin = n.thin, n.burnin = n.burn,
...
lgrmar2c.out2 <- R2WinBUGS::bugs(data = data, inits = NULL,
                                 parameters.to.save = monitor,
                                 model.file = file.path(getwd(), "lgrmar2c.txt"),
                                 n.iter = n.sim, n.thin = n.thin, n.burnin = n.burn,
                                 bugs.directory = wbd,  working.directory = getwd(),
...
monitor <- c("delta", "alpha", "kappa", "theta", "rho")
n.burn <- 4000
n.thin <- 10
n.sim <- 500 * n.thin + n.burn

lgrmar2d.out <- R2OpenBUGS::bugs(data = data, inits = NULL,
                                 parameters.to.save = monitor,
                                 model.file = file.path(getwd(), "lgrmar2d.txt"),
                                 n.iter = n.sim, n.thin = n.thin, n.burnin = n.burn,
                                 OpenBUGS.pgm = obd,  working.directory = getwd(),
                                 debug = TRUE,
                                 codaPkg = FALSE)

lgrmar2d.out <- R2WinBUGS::bugs(data = data, inits = NULL,
                                 parameters.to.save = monitor,
                                 model.file = file.path(getwd(), "lgrmar2d.txt"),
                                 n.iter = n.sim, n.thin = n.thin, n.burnin = n.burn,
                                 bugs.directory = wbd,  working.directory = getwd(),
...
lgrmar2d.out2 <- R2WinBUGS::bugs(data = data, inits = NULL,
                                 parameters.to.save = monitor,
                                 model.file = file.path(getwd(), "lgrmar2d.txt"),
                                 n.iter = n.sim, n.thin = n.thin, n.burnin = n.burn,
                                 bugs.directory = wbd,  working.directory = getwd(),
